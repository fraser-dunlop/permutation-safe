-- | Internal module to hide implementation details
...
instance (Eq a) => Semigroup (Permutation a) where
  (<>) pl@(Permutation l) pr@(Permutation r) =
     ...
     in case fromRelation (zip elemsofp (permfunc <$> elemsofp)) of
          Left _  -> error $ "Data.Permutation.Semigroup: this should only happen "
...
toRelation :: (Eq a, Enum a)
           => a -- ^Lower bound 
           -> a -- ^Upper bound
           -> Permutation a -> Either PermutationError [(a,a)]
...
-- | Given a lower and upper bound on a and a permutation within this range
-- returns a two line for representation. 
toTwoLineForm :: (Eq a, Enum a)
              => a -- ^Lower bound
              -> a -- ^Upper bound
              -> Permutation a -> Either PermutationError ([a],[a])
toTwoLineForm from to p =
  case toRelation from to p of
    Left _ -> Left $ PermutationError
...
mapsOnto i = do
  (_,_,m) <- get
  case lookup i m of
    Nothing ->error $ "Data.Permutation.toCycles: this should only happen "
                   ++ "if you didn't use a smart constructor AND created a "
                   ++ "Permutation object that is not in fact a permutation.\n"
                   ++ "If you did use a smart constructor and you see this "
                   ++ "error then please submit a bug report with a minimal "
...
nextCycleElem = do
  (w,c,m) <- get
  let last = head $ reverse w
  next <- mapsOnto last
  let filt = filter (/= (last, next)) m
  if next == head w
     then put ([], w:c, filt)
